  ...
  return (
    <div className="space-y-3">
      {checks.map((check) => {
        ...
        return (
          <div key={`${check.rule}-${check.message}`} className="flex min-w-0 gap-3 rounded-lg border border-white/10 bg-white/[0.035] p-3">
            <div
              className={cn(
            ...
            <div className="min-w-0">
              <p className="break-words text-sm font-medium text-white">{check.rule}</p>
              <p className="mt-1 break-words text-sm leading-6 text-slate-400">{check.message}</p>
            </div>
          </div>
        );
      })}
