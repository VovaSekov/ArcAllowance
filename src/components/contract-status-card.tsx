...
export function ContractStatusCard({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <section className="min-w-0 rounded-lg border border-violet-400/20 bg-violet-400/10 p-5">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div className="flex min-w-0 gap-3">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <h2 className="break-words text-lg font-semibold text-white">Real Arc Testnet contract</h2>
        ...
        <Link href="/contract" className="inline-flex shrink-0 items-center justify-center gap-2 rounded-md border border-violet-300/30 px-3 py-2 text-sm font-medium text-violet-50 hover:bg-violet-300/10">
          Contract page
      ...
      <dl className={compact ? "mt-4 grid gap-3 text-sm sm:grid-cols-2 xl:grid-cols-3" : "mt-5 grid gap-3 text-sm sm:grid-cols-2 xl:grid-cols-4"}>
        ...
        <div>
          ...
          <dd className="mt-1 break-all font-mono text-xs text-white">{configured ? shortAddress(arcAllowanceRegistry.address) : "Not deployed yet"}</dd>
        </div>
        {!compact ? (
          <div>
