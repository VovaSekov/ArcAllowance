...
export function AgentCard({ agent, policy }: { agent: Agent; policy?: Policy }) {
  return (
    <div className="min-w-0 rounded-lg border border-white/10 bg-white/[0.045] p-5 transition hover:border-sky-400/30 hover:bg-white/[0.06]">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <h3 className="break-words text-lg font-semibold text-white">{agent.name}</h3>
      ...
      <div className="mt-5 rounded-md border border-white/10 bg-ink-950/60 p-3">
        <div className="flex min-w-0 items-center gap-2 text-sm text-slate-300">
          <WalletCards className="h-4 w-4 shrink-0 text-sky-300" aria-hidden="true" />
          <span className="break-all font-mono text-xs">{shortAddress(agent.walletAddress)}</span>
        </div>
